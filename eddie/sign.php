...
<div id="addPin" class="modal hide fade in" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true" style="display:block;">
	...
	<button type="button" class="close closed none" data-dismiss="modal" aria-hidden="true">×</button>
    ...
    <div class="modal-footer">
    	<button class="btn dead closed none" data-dismiss="modal" aria-hidden="true">Close</button>
...
<section id="iframe" class="clearfix wrap">
...
<article class="center">
	<p><img src="uploads/documents/<?= $doc->client_id; ?>/<?= $doc->document; ?>" /></p>
</article>
<? else: ?>
<article>
<iframe src="https://docs.google.com/viewer?url=<?= $url; ?>&embedded=true" width="100%" height="500" style="border: none;"></iframe>
...
<article>
    ...
    <h4>Please sign directly below if you agree to the document viewed.</h4>
	<div id="signature"></div>
    <button id="save" class="btn" onclick="importImg($('#signature'))">Save Signature</button><span id="save-msg"></span>
    
</article>

<div id="message"></div> <!-- end message -->

<form id="sign" class="hide">
<input type="hidden" name="document_id" value="<?= $doc->id; ?>" />
<label class="left forty right-1">Please Type Your Full Name<br> <input type="text" name="full_name" class="default" /></label>
<label class="left forty clearfix"><span class="left block">Pin Number</span><br /><input type="password" name="pin" id="pin" class="left twenty right-1" /> <a href="#" class="left resetPin">Forgot Pin</a></label>
<div class="clearfix"></div><!-- Clear -->
<label class="left twenty">Date Signed<br> <input type="text" name="date" value="<?= date('m/d/Y'); ?>" class="default" readonly /></label>
<div class="clearfix"></div><!-- Clear -->
<input type="submit" class="btn btn-primary btn-large submit" value="Send Signature" /> <img src="img/ajax-loader.gif" class="none loading" />
</form>
			...
			$.ajax({
				...
				success: function(data) {
					if (data.status == 'error') { 	
						...
						$('#pin').val(i);
						$('#addPin .closed').removeClass('none');
					} // end else		 
				},
				dataType: 'json'
			}); 	 			 
